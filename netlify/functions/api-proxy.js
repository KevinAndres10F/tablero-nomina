...
exports.handler = async (event) => {
  // 1. Correo Gmail OAuth2 (no necesita backend).
  ...
  // 3. Todo lo demas (telemetria, chat, etc.) -> se proxiea al backend si existe.
  const backendOrigin = normalizeBackendOrigin(process.env.BACKEND_API_ORIGIN);
  if (!backendOrigin) {
    return jsonResponse(404, {
      ok: false,
      error: "Endpoint no soportado en modo Excel",
  ...
  const targetUrl = pathname
    ? `${backendOrigin}/api/${pathname}${query}`
